...
**4. Generate kernel.** Fill `templates/kernel.md` → `AGENTS.md` (the cross-agent standard file — Codex, Cursor, Gemini CLI, OpenCode and most agents read it natively), plus a one-line `CLAUDE.md` next to it containing exactly `@AGENTS.md` (Claude Code imports the kernel through it). The routing table must list **only files that exist**, guard rails must quote the user's own numbers and named patterns. Keep every standing-rule block from the template. Wherever the kernel (or any file) tells future sessions to consult an external source (calendar, tracker, app), it must state the recorded access path — or "not connected — ask the user" if there is none.

**5. Handoff.** Populate `index.md` with every created file; write the first `log.md` entry (`structure | vault bootstrapped`). Walk the user through the loops in plain words: capture → inbox processing, planning cadence + retrospectives, decision filing, passive learning. Offer to create the first weekly plan now as a live demo, and record the outcome (created or declined) in the bootstrap log entry so the next session knows whether to re-offer. Close by recommending the user put the folder under some history layer of their choice (git, or whatever sync they use) — versioning is orthogonal to the mechanics, but the log's "routine tweaks" bar assumes one exists.
...
- [ ] No folder is empty (READMEs in inbox/, archive/, …); every external-source mention states its access path or "not connected"
- [ ] Bootstrap log entry records the first-plan offer outcome
- [ ] No invented facts — unknowns marked `⚠️ to capture`; quotes normalized of dictation artifacts
